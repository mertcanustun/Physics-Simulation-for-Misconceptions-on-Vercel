...
## OPSIYONEL sunucu kopyası (bkz. scripts/Uplink.gd): olaylar yığın hâlinde
## /api/collect adresine gönderilir. Yerel JSONL kaydı HER ZAMAN asıl kayıttır;
## gönderim başarısız olsa bile hiçbir veri kaybolmaz ve simülasyon etkilenmez.
const UPLINK_BATCH := 25            # kaç olayda bir sunucuya gönderilsin
...
var _uplink_buf: Array = []         # sunucuya henüz gönderilmemiş olaylar
...
func end_session() -> void:
	if _active():
		_write("session_end", {})
	_uplink_flush()   # oturum kapanırken kalan olayları da gönder
	_close()
	_sid = ""
	_deciding = false
...
func run_complete(goal: bool, impact_x: float, category := "") -> void:
	...
	_uplink_flush()   # deneme bitti: sekme kapansa bile bu yığın gitmiş olsun
...
func _write(type: String, payload: Dictionary) -> void:
	...
	row.merge(payload)
	_uplink_buf.append(row)
	if _uplink_buf.size() >= UPLINK_BATCH:
		_uplink_flush()
	_open()
	if _f == null:
...
## Biriken olayları sunucuya gönder (yalnız web; başarısızlık sessizce yutulur).
func _uplink_flush() -> void:
	if _uplink_buf.is_empty():
		return
	var batch := _uplink_buf.duplicate()
	_uplink_buf.clear()
	Uplink.post("events", batch)
