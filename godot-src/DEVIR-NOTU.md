...
- Sabit atış (deneysel kontrol): **v0 = 30 m/s, açı = 45 derece** - kullanıcı hız/açı seçemez; sonucu yalnız kuvvet seçimi belirler.
- Kuvvetler:
  ...
  - **Vuruş kuvveti F (impetus):** `acc += vel.normalized() * IMPETUS_ACC` (15 m/s^2) - **KASITLI YANLIŞ FİZİK** (ortaçağ impetus yanılgısı: "temas bitince de itmeye devam eder"). Bu bir bug DEĞİL, pedagojinin kalbi. **DÜZELTME.**
...
## 4. İLK GÖREV: fizik doğruluğu denetimi - [TAMAM] TAMAMLANDI (2026-08-07)
...
4. **Menzil kontrolü:** v0=30, 45 derece, dragsız menzil ~= 91.7 m; DRAG_MED ile ne oluyor, hedef (`target_x`) makul bir sahaya denk geliyor mu?
