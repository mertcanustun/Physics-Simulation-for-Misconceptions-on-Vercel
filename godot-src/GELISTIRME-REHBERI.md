...
- **PixelLab** (pixellab.ai) - 8-bit futbolcu karakteri + idle/koşu/vuruş animasyonları
  (Character Creator, v3, Sidescroller, east yönü, 124x124, şeffaf PNG kareler).
...
|---|---|---|---|
| `player_idle.png` | Futbolcu ayakta, yandan, **sağa** bakar | ~64x128 px | Ayaklar en altta (taban hizası sabit) |
| `player_run.png` | Koşu animasyonu **spritesheet** | 6 kare yan yana (ör. 6x64=384 x 128) | Giriş koşusunda oynatılacak |
| `player_kick.png` | Vuruş anı (bacak uzanmış) | ~64x128 px | 1 kare yeter (istersen 2) |
| `ball.png` | Futbol topu, yandan | 64x64 px | Yuvarlak + pentagonlar; dönüşü kodla yaparız |
| `sky_stadium.png` | Gökyüzü + bulanık tribün (arka plan) | 1920x1080 (geniş) | En arkada durur; **çim İÇERMESİN** (çim kodla çizilip kayıyor) |

İsteğe bağlı (etkisi yüksek ama şart değil):
| `grass_tile.png` | Tekrarlanabilir çim dokusu | 256x256, seamless | Düz yeşil yerine doku |
| `goal_frame.png` | Kale direği + üst kiriş (yandan) | ~200x260 px | **Ağ (file) DAHİL ETME** - file kodla çiziliyor ki şişebilsin |
...
- `|vy| < APEX_VY` olduğunda `apex_flash` tetiklenir ve rozet çizilir:
  **"TEPE NOKTASI / dikey hız vy = 0 / ama ivme asagi yonde devam ediyor"**.
...
- ÖNEMLİ: 45 derece'lik vuruşta tepede TOPLAM hız sıfır değildir, yalnızca dikey
...
- Alt çubukta 0.25x / 0.5x / 1x / 2x düğmeleri.
...
## Doğrulanmış fizik değerleri (v0=30 m/s, 45 derece)
...
- Sol üstteki eski sabit "BILGI Nasıl çalışır?" kartı (`howto_card`) tamamen
...
3. **Sayı/renk/ses değiştirmek için:** `config/` klasörünü açın,
   ...
   - Ses alanları (Kick Sfx/Wind Sfx): sağdaki küçük ok v -> "Load" ile
     `assets/audio/` altından (veya başka bir yerden) bir ses dosyası seçin.
   - Bir değeri eskiye döndürmek isterseniz alanın solundaki küçük geri al okuna
...
5. Üstteki Play (oynat) düğmesine basıp (veya F5) deneyin.
...
`imp - sin45 derece = 0.707-imp`. `imp = 22.5` iken bu 15.9 m/s^2 > g = 9.81 -> net
...
### Doğrulanmış son değerler (v0=30 m/s, 45 derece, drag_k=0.025, kale 34.0 m)
